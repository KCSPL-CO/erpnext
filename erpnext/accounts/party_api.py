...
def list_party_type():
    return get_list_api("Party Type")

# ---- Get Details ----
@frappe.whitelist(allow_guest=True)
def get_details_party_type():
    if frappe.request.method != "GET":
        frappe.local.response["http_status_code"] = 405
        return {"error": "Only GET method allowed"}
    ...
    try:
        doc = frappe.get_doc("Party Type", name)
        return {"data": doc.as_dict()}
    except frappe.DoesNotExistError:
        frappe.local.response["http_status_code"] = 404
        return {"error": f"Party Type '{name}' not found"}
    except Exception as e:
        frappe.log_error(frappe.get_traceback(), "Get Party Type Error")
        frappe.local.response["http_status_code"] = 500
        return {"error": str(e)}

@frappe.whitelist(allow_guest=True)
def list_address():
    return get_list_api("Address")

@frappe.whitelist(allow_guest=True)
def list_project():
    return get_list_api("Project")

@frappe.whitelist(allow_guest=True)
def list_cost_center():
    return get_list_api("Cost Center")

@frappe.whitelist(allow_guest=True)
def list_docType():
    return get_list_api("DocType")
